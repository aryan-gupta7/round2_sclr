"""Evaluate the trained adapter vs the base model on held-out seeds."""
import os
import sys

# We need to run this on HF jobs, so this script will load vLLM/Unsloth
code = """
import torch
...
with open("/home/s1nn3r/Documents/sclr_round2/training/eval_hf.py", "w") as f:
    f.write(code)
...
command = [
    'bash', '-c',
    "echo '{script_b64}' | base64 -d > eval_hf.py && "
...
print(f"https://huggingface.co/jobs/s1nn3rx69/{{job.job_id}}")
"""

with open("/home/s1nn3r/Documents/sclr_round2/training/submit_eval.py", "w") as f:
    f.write(submit_code)
