...
# OpenEnv Development Rules

These rules must be followed when designing or implementing new environments within this repository.

## 1. Directory Structure and Naming

- **Repository Path**: All environments must reside in `envs/<name>_env/`.
- **Normalization**: Use `snake_case` for all directory and file names.
- **Components**:
  - `envs/<name>_env/models.py`: Pydantic models for Action and Observation.
  - `envs/<name>_env/client.py`: `EnvClient` implementation.
  - `envs/<name>_env/server/`: Server-side logic.
  - `envs/<name>_env/server/<name>_environment.py`: Core environment logic.
  - `envs/<name>_env/server/app.py`: FastAPI server entry point.

## 2. Coding Patterns

- **Dual-Import Pattern**: (CRITICAL) Required in `app.py` and environment files to ensure compatibility between local development (`PYTHONPATH=src:envs`) and Docker runtimes (`PYTHONPATH=/app/env`).
  ```python
  try:
      ...
      from models import MyAction, MyObservation
  ```
- **Class Naming**: Use `PascalCase` for environment, action, and observation classes (e.g., `TextArenaEnvironment`, `TextArenaAction`).
- **Environment Contract**:
  - `reset(seed=None, episode_id=None, **kwargs) -> ObservationType`
  - `step(action, timeout_s=None, **kwargs) -> ObservationType`
  - Set `SUPPORTS_CONCURRENT_SESSIONS = True` if the environment isolates state between instances.

## 3. Server Implementation

- **App Factory**: Use `create_app` inside `server/app.py` and pass the environment **class** (not an instance) to support concurrent sessions.
  ```python
  app = create_app(MyEnvironment, MyAction, MyObservation, env_name="my_env")
  ```
- **Isolation**: When `SUPPORTS_CONCURRENT_SESSIONS` is `True`, set `max_concurrent_envs > 1` in `create_app`.

## 4. Environment Configuration

- **Dependencies**: Add all external libraries to the `dependencies` list in `pyproject.toml`.
- **Manifest**: Keep `openenv.yaml` aligned with the standard format (spec_version: 1).
- **README**: Every environment MUST have a `README.md` with:
  - Quick Start example.
  - Action/Observation field descriptions.
  - Reward design details.

## 5. Validation Workflow

Before finishing a task, always validate the environment:

1. **Import Check**: `PYTHONPATH=src:envs uv run python -c "from envs.<name>_env.server.<name>_environment import <ClassName>Environment"`
2. **Server Health**: Start the server with `uvicorn` and check `curl http://localhost:8000/health`.
3. **OpenEnv Validate**: Run `openenv validate --url http://localhost:8000`.
